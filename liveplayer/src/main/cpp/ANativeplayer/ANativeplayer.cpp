...
void av_log_txt(void *ptr, int level, const char *fmt, va_list vl) {
...
Java_com_tz_dream_sdl_ffmpeg_demo_ANativeWindowPlayerActivity_play(JNIEnv *env, jclass type,
    ...
    if (videoStream == -1) {
        LOG_I("Didn't find a video stream.");
    ...
    if (pFrameRGBA == NULL || pFrame == NULL) {
        LOG_I("Could not allocate video frame.");
        return -1;
    }
...
Java_com_tz_dream_sdl_ffmpeg_demo_ANativeWindowPlayerActivity_decode(JNIEnv *env, jclass type,
    ...
    av_log_set_callback(av_log_txt);
    ...
    while (av_read_frame(pFormatCtx, packet) >= 0) {
        if (packet->stream_index == videoindex) {
            ...
            if (got_picture) {
                ...
                LOG_E("width=%d;height=%d",pCodecCtx->width,pCodecCtx->height);
                ...
                LOG_I("Frame Index: %5d. Type:%s", frame_cnt, pictype_str);
                frame_cnt++;
            }
        }

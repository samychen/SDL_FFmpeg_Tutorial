...
double get_audio_clock(VideoState *is) {
    ...
    bytes_per_sec = 0;
    n = is->audio_frame->channels * 2;
    if(is->audio_st) {
        bytes_per_sec = is->audio_frame->sample_rate * n;
    }
    if(bytes_per_sec) {
...
int audio_decode_frame2(VideoState *is) {
...
int audio_decode_frame(VideoState *is, uint8_t *audio_buf, int buf_size, double *pts_ptr) {
    ...
    int n;
    uint8_t *out_buffer;
    size_t size=AVCODEC_MAX_AUDIO_FRAME_SIZE*2;
    int64_t in_ch_layout = av_get_default_channel_layout(is->audio_st->codec->channels);
    out_buffer = (uint8_t *)av_malloc(size);
    au_convert_ctx = swr_alloc();
    au_convert_ctx= swr_alloc_set_opts(au_convert_ctx,AV_CH_LAYOUT_STEREO,AV_SAMPLE_FMT_S16,is->audio_frame->sample_rate,
                                       in_ch_layout,is->audio_st->codec->sample_fmt,is->audio_frame->sample_rate,0,NULL);
    ...
    for(;;) {
        while(is->audio_pkt_size > 0) {
            ...
            len2 = swr_convert(is->swr_ctx, &out_buffer,
                               sizeof(is->audio_buf2) / is->audio_tgt_channels
                               / av_get_bytes_per_sample(is->audio_tgt_fmt),//MAX_AUDIO_FRAME_SIZE
                               is->audio_frame->extended_data, is->audio_frame->nb_samples);
            if (len2 < 0) {
                fprintf(stderr, "swr_convert() failed\n");
                break;
            }
...
//            audio_data_size = audio_decode_frame(is,is->audio_buf, sizeof(is->audio_buf), &pts);
            audio_data_size = audio_decode_frame2(is);
...
double synchronize_video(VideoState *is, AVFrame *src_frame, double pts) {
    ...
    printf("video_clock=%lf",is->video_clock);
...
int audio_stream_component_open(VideoState *is, int stream_index) {
    ...
    if (wanted_spec.freq <= 0 || wanted_spec.channels <= 0) {
        LOG_I(stderr, "Invalid sample rate or channel count!\n");
        return -1;
    }
    ...
    while (SDL_OpenAudio(&wanted_spec, &spec) < 0) {
        LOG_I(stderr, "SDL_OpenAudio (%d channels): %s\n",
    ...
    if (spec.format != AUDIO_S16SYS) {
        LOG_I(stderr, "SDL advised audio format %d is not supported!\n",
              spec.format);
        return -1;
    }
    ...
    if (spec.channels != wanted_spec.channels) {
        ...
        if (!wanted_channel_layout) {
            LOG_I(stderr, "SDL advised channel count %d is not supported!\n",
                  spec.channels);
            return -1;
        }
    }
    ...
    if (!codec || (avcodec_open2(codecCtx, codec, NULL) < 0)) {
        LOG_I(stderr, "Unsupported codec!\n");
        return -1;
    }
...
int decode_thread(void *arg) {
    ...
    if (avio_open2(&is->io_ctx, is->filename, 0, &interupt_cb, NULL)) {
        LOG_I(stderr, "Cannot open I/O for %s\n", is->filename);
        return -1;
    }
    ...
    if (is->videoStream < 0 || is->audioStream <= 0) {
        LOG_I(stderr, "%s: could not open codec\n", is->filename);
        goto fail;
    }
...
int main(int argc, char *argv[]) {
    ...
    if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO | SDL_INIT_TIMER)) {
        LOG_I(stderr, "Could not initialize SDL - %s\n", SDL_GetError());
        exit(1);
    }
